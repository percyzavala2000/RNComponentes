...
export const CustomSectionListScreen = () => {
  ...
  return (
    <CustomView margin>
      <Title title="Lista de Personajes" safe />
      <Card>
        <SectionList
          sections={houses}
          keyExtractor={(item) => item}
          renderItem={item => (
            <Text style={{marginVertical: 2}}>{item.item}</Text>
          )}
          ...
          ListHeaderComponent={()=><Title title="Personajes"/>}
          ListFooterComponent={()=><Title title={`Fin de secciones total: ${houses.length}`}/>}
          style={{height:height-top-20}}
        />
      </Card>
    </CustomView>
  );
